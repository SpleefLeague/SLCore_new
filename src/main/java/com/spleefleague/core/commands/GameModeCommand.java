...
public class GameModeCommand extends CommandTemplate {
    ...
    @CommandAnnotation
    public void gameMode(CorePlayer sender, @HelperArg(value="<0-3>") Integer i) {
    ...
    @CommandAnnotation
    public void gameMode(CorePlayer sender, CorePlayer cp, @HelperArg(value="<0-3>") Integer i) {
        GameMode gm = getGameMode(i);
        if (gm != null) {
            cp.setGameMode(gm);
            if (!cp.equals(sender))
